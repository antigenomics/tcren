...
FOOTPRINT_FEATURES: tuple[str, ...] = (
    *FOOTPRINT_SIZE_FEATURES,
    ...
    "h0_pers_ent",
    *GRAPH_FEATURES, *PROMOTED_POSE_FEATURES, *MATRIX_FEATURES,
)
...
def footprint_features(structure: Structure, *, cutoff: float = 5.0,
    ...
    # The same contact map read as a graph, and the Calpha/Cbeta maps read as matrices. Both are
    # computed unconditionally: unlike the cell partition they have no empty-tally branch, and each
    # returns its own all-NaN row when the structure does not support it.
    row.update(graph_features(structure, cutoff=cutoff))
    row.update(matrix_features(structure, cutoff=cutoff))
    return row
...
def footprint_batch(structures: str | Path | Iterable[Structure], *, cutoff: float = 5.0,
    ...
    if isinstance(structures, (str, Path)):
        ...
        from ..annotation.batch import iter_typed
        from ..mhc import annotate_mhc_batch

        structs = list(iter_typed(Path(structures), organism=organism))
