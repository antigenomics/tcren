...
therefore orders the redundancy structure of \cref{sec:quotient}. Dashed lines attach the six
families to the objects they read; \emph{topology} reads four of them, which is why it is the
...
\draw[->] (-0.3,0) -- (9.4,0) node[right,font=\scriptsize] {$x$};
\draw[->] (-0.3,-1.5) -- (-0.3,1.9) node[above,font=\scriptsize] {$h$ [\AA]};
% the pMHC face
\draw[thick,orange!75!black]
  plot[smooth,domain=0:9,samples=60] (\x, {0.42*sin(\x r*1.15) + 0.18*sin(\x r*3.1)});
\node[orange!65!black,font=\scriptsize,anchor=west] at (9.05,-0.55) {$h_{\mathrm{pMHC}}$};
% the TCR face
\draw[thick,blue!65!black]
  plot[smooth,domain=0:9,samples=60] (\x, {0.36*sin(\x r*1.15 + 40) - 0.28});
\node[blue!60!black,font=\scriptsize,anchor=west] at (9.05,0.42) {$h_{\mathrm{TCR}}$};
% shade: interlock where TCR below pMHC
\begin{scope}
\clip plot[smooth,domain=0:9,samples=80] (\x, {0.42*sin(\x r*1.15) + 0.18*sin(\x r*3.1)})
      -- (9,-2) -- (0,-2) -- cycle;
\fill[blue!22] plot[smooth,domain=0:9,samples=80] (\x, {0.36*sin(\x r*1.15 + 40) - 0.28})
      -- (9,2) -- (0,2) -- cycle;
\end{scope}
% shade: void where TCR above pMHC
\begin{scope}
\clip plot[smooth,domain=0:9,samples=80] (\x, {0.36*sin(\x r*1.15 + 40) - 0.28})
      -- (9,-2) -- (0,-2) -- cycle;
\fill[orange!25] plot[smooth,domain=0:9,samples=80]
      (\x, {0.42*sin(\x r*1.15) + 0.18*sin(\x r*3.1)}) -- (9,2) -- (0,2) -- cycle;
\end{scope}
\node[font=\scriptsize,anchor=west] at (0.15,1.55)
  {\tikz\fill[blue!22](0,0)rectangle(2.6mm,2.6mm); \ \dsc{sc\_interlock} $=\!\int\!\max(0,-\mathrm{gap})\,\mathrm{d}A$};
\node[font=\scriptsize,anchor=west] at (5.05,1.55)
  {\tikz\fill[orange!25](0,0)rectangle(2.6mm,2.6mm); \ \dsc{sc\_gap\_vol} $=\!\int\!\max(0,\mathrm{gap})\,\mathrm{d}A$};
\node[font=\scriptsize,anchor=west] at (0.15,-1.28)
  {median cell interdigitates: \dsc{sc\_interlock\_frac} $=0.750$ over 371 crystals};
\end{tikzpicture}
...
% groove floor
\draw[fill=gray!12,draw=gray!55] (0,0) ellipse (3.5 and 0.85);
\node[gray!60!black,font=\scriptsize] at (0,-0.62) {groove floor C$\alpha$ (SVD)};
% helices
\draw[very thick,orange!70!black,decorate,decoration={coil,aspect=0,segment length=4.6mm,amplitude=1.1mm}]
  (-3.0,0.62) -- (3.0,0.62);
\draw[very thick,orange!70!black,decorate,decoration={coil,aspect=0,segment length=4.6mm,amplitude=1.1mm}]
  (-3.0,-0.62) -- (3.0,-0.62);
\node[orange!65!black,font=\scriptsize,anchor=west] at (3.1,0.62) {$\alpha$1 helix};
\node[orange!65!black,font=\scriptsize,anchor=west] at (3.1,-0.62) {$\alpha$2 ($\beta$1) helix};
% peptide
\draw[thick,green!45!black,-{Stealth[length=2.4mm]}] (-2.3,0.06) -- (2.3,0.06);
\foreach \x in {-2.0,-1.3,-0.6,0.1,0.8,1.5,2.1}
  \fill[green!45!black] (\x,0.06) circle (1.5pt);
\node[green!40!black,font=\scriptsize] at (2.55,0.32) {C};
\node[green!40!black,font=\scriptsize] at (-2.55,0.32) {N};
% TCR body
\draw[fill=blue!12,draw=blue!55,rounded corners=3pt] (-1.35,1.55) rectangle (1.35,2.55);
\node[blue!55!black,font=\scriptsize] at (0,2.05) {V$\alpha$\,/\,V$\beta$};
\draw[blue!55] (-0.75,1.55) .. controls (-0.5,1.05) and (-0.3,0.75) .. (-0.35,0.42);
\draw[blue!55] (0.75,1.55) .. controls (0.5,1.05) and (0.3,0.75) .. (0.42,0.42);
\node[blue!55!black,font=\scriptsize,anchor=west] at (0.55,0.95) {CDR loops};
% the frame
\draw[-{Stealth[length=2.6mm]},very thick] (0,0.06) -- (1.5,0.06)
  node[below right,font=\scriptsize] {$\hat u$ \ groove long axis};
\draw[-{Stealth[length=2.6mm]},very thick] (0,0.06) -- (-0.62,-0.62)
  node[below left,font=\scriptsize] {$\hat w$};
\draw[-{Stealth[length=2.6mm]},very thick] (0,0.06) -- (0,1.35)
  node[left,font=\scriptsize,pos=0.85] {$\hat n$};
\fill (0,0.06) circle (1.6pt);
% height
\draw[<->,gray!70] (2.75,0.06) -- (2.75,1.9) node[midway,right,font=\scriptsize,black]
  {\dsc{height}};
\draw[gray!55,dashed] (0,1.9) -- (2.75,1.9);
\end{tikzpicture}
